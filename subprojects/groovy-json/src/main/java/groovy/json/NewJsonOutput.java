    ...
    private static void writeObject(Object object, CharBuf buffer) {
        if (object == null) {
            ...
            if (CharSequence.class.isAssignableFrom(objectClass)) { // Handle String, StringBuilder, GString and other CharSequence implemenations
                ...
                buffer.addQuoted(dateFormatter.get().format((Date) object));
            } else if (Calendar.class.isAssignableFrom(objectClass)) {
                buffer.addQuoted(dateFormatter.get().format(((Calendar) object).getTime()));
                ...
                buffer.addJsonEscapedString(((URL) object).toString());
            } else if (objectClass == UUID.class) {
                buffer.addQuoted(((UUID) object).toString());
            } else if (Closure.class.isAssignableFrom(objectClass)) {
                writeMap(NewJsonDelegate.cloneDelegateAndGetContent((Closure<?>) object), buffer);
                ...
                Iterable<?> iterable = Collections.list((Enumeration<?>) object);
                writeObject(iterable, buffer);
